...
G4VPhysicalVolume* musrDetectorConstruction::Construct()  {
  ...
  while (!feof(fSteeringFile)) {
	...
	if (strcmp(storeIt,"off")==0) {
	  ...
	  if (strcmp(tmpString2,"parIniTime")==0)   {musrRootOutput::store_parIniTime = false;}   // Y. ZENG 23 Apr 2024
	  if (strcmp(tmpString2,"parIniPosX")==0)   {musrRootOutput::store_parIniPosX = false;}   // Y. ZENG 23 Apr 2024
	  if (strcmp(tmpString2,"parIniPosY")==0)   {musrRootOutput::store_parIniPosY = false;}   // Y. ZENG 23 Apr 2024
	  if (strcmp(tmpString2,"parIniPosZ")==0)   {musrRootOutput::store_parIniPosZ = false;}   // Y. ZENG 23 Apr 2024
	  if (strcmp(tmpString2,"parIniMomX")==0)   {musrRootOutput::store_parIniMomX = false;}   // Y. ZENG 23 Apr 2024
	  if (strcmp(tmpString2,"parIniMomY")==0)   {musrRootOutput::store_parIniMomY = false;}   // Y. ZENG 23 Apr 2024
	  if (strcmp(tmpString2,"parIniMomZ")==0)   {musrRootOutput::store_parIniMomZ = false;}   // Y. ZENG 23 Apr 2024
	  if (strcmp(tmpString2,"parID")==0)        {musrRootOutput::store_parID = false;}        // Y. ZENG 23 Apr 2024
...
//
  ...
  SteelSlag->AddElement(Si, 0.19);  // ~19% Silicon Dioxide (SiO₂)
  SteelSlag->AddElement(Fe, 0.25);  // ~25% Iron Oxide (Fe₂O₃)
  SteelSlag->AddElement(Al, 0.06);   // ~6% Aluminum Oxide (Al₂O₃)
  SteelSlag->AddElement(Mg, 0.08);   // ~8% Magnesium Oxide (MgO)
  ...
  CalciumCarbonate->AddElement(C, 1);  // One part Carbon
  CalciumCarbonate->AddElement(O, 3);  // Three parts Oxygen
  ...
  cortenSteel->AddElement(C,  0.002);
  cortenSteel->AddElement(Mn, 0.01);
  cortenSteel->AddElement(Si, 0.005);
  cortenSteel->AddElement(Cu, 0.01);
  cortenSteel->AddElement(Cr, 0.003);
  
  //intermodal container flooring
  G4Material* wood = new G4Material("Wood", density = 0.7 * CLHEP::g / CLHEP::cm3, ncomponents=3);
  wood->AddElement(C, 0.50);
  wood->AddElement(H, 0.06);
  wood->AddElement(O, 0.44);
